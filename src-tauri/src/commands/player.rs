...
/// Append songs to the "play next" queue without disturbing current playback.
#[tauri::command]
pub async fn queue_songs(song_ids: Vec<i64>, state: State<'_, AppState>) -> Result<(), String> {
    use rusqlite::params;
    let conn = state.db.pool.get().map_err(|e| e.to_string())?;
    ...
    let items = songs
        .into_iter()
        .map(|s| PlaylistItem::new_song(0, 0, s))
        .collect::<Vec<_>>();

    let mut player = state.player.lock().await;
    player.queue_items(items);
    Ok(())
}
