...
- For debugging a frontend issue (console errors, live DOM/CSS state), launch with `LUMINOUS_REMOTE_DEVTOOLS=1 bun run tauri dev` and attach the Browser pane to `http://127.0.0.1:9222` instead of asking the user to describe what they see — see [docs/TESTING.md](docs/TESTING.md)'s "Remote devtools for headless/agent debugging" section.
