...
<footer transition:fly={{ y: 40, duration: 300, easing: cubicOut }} class="h-20 max-w-[1400px] mx-auto bg-brand-playerbar border border-brand-border rounded-[2rem] flex items-center justify-between px-8 text-brand-text-secondary select-none {themeStore.isGlassTheme || isLinux ? 'glass-surface' : ''} {isLinux ? 'opaque-linux' : ''}">
  <!-- Track Metadata & Art -->
